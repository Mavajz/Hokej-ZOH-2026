...
# --- 2. DATA (Aktualizováno po debaklu Česka s Kanadou) ---
team_powers = {
    "Kanada": 99, "USA": 97, "Švédsko": 92, "Česko": 84,   # Česko dolů o 5 bodů
    "Slovensko": 84, "Švýcarsko": 84, "Finsko": 82,      # Česko je teď na úrovni SVK a SUI
    "Německo": 76, "Dánsko": 60, "Lotyšsko": 58,
    "Itálie": 40, "Francie": 33
}

# REÁLNÉ VÝSLEDKY - Historie turnaje
real_results = {
    ("Slovensko", "Finsko"): (4, 1, "REG"),
    ("Švédsko", "Itálie"): (5, 2, "REG"),
    ("Švýcarsko", "Francie"): (4, 0, "REG"),
    ("Česko", "Kanada"): (0, 5, "REG") # NOVÝ VÝSLEDEK
}
...
# --- 3. CSS DESIGN (Dark Mode & Professional Look) ---
st.markdown("""
<style>
    .match-box {
...
# --- 4. FUNKCE PRO SIMULACI (IIHF Pravidla) ---
...
def sim_match(t1, t2, m_seed):
    ...
    rtype = "REG"
    if s1 == s2:
        rtype = "OT"; s1, s2 = (s1+1, s2) if random.random() < (p1/(p1+p2)) else (s1, s2+1)
    return s1, s2, rtype
...
def run_tourney_cached(seed):
    ...
    for i, (h, l) in enumerate([(4,11),(5,10),(6,9),(7,8)]):
        ...
        matches.append({"d": "Úterý 17. 2.", "t1": t1, "t2": t2, "s1": s1, "s2": s2, "rt": rt, "stg": "PO", "lbl": f"OF{i+1}", "w": of_w[h]})
    qf_w = []
...
with tab1:
    ...
    with c_ctrl2: sel_date = st.select_slider("Časová osa", options=dates_list)
    ...
    if today:
        ...
        for row in rows:
            ...
            for i, m in enumerate(row):
                with cols[i]:
                    ot = "<span class='ot'>PRODL.</span>" if m["rt"] == "OT" else ""
                    st.markdown(f"<div class='match-box'><div class='team-n'>{m['t1']}</div><div class='score-n'>{m['s1']}:{m['s2']}</div><div class='team-n' style='text-align:right;'>{m['t2']}</div></div>", unsafe_allow_html=True)
    ...
    if date_idx <= 4:
        ...
        for gn, teams in groups_def.items():
            ...
            df = pd.DataFrame([{"Tým": t, "B": g_stats[t]["B"], "Skóre": f"{g_stats[t]['GF']}:{g_stats[t]['GA']}"} for t in sorted_tms])
            df.index += 1
        ...
        for i, code in enumerate(["OF", "ČF", "SF", "Medal"]):
            with [c_of, c_qf, c_sf, c_fin][i]:
                ...
                for m in [x for x in po if code in x.get("lbl", "") or (code=="Medal" and x["stg"]=="PO" and x["lbl"] in ["BRONZ", "FINÁLE"])]:
                    st.markdown(f"<div class='bracket-card'>{m['t1']} - {m['t2']} <br><b>{m['s1']}:{m['s2']}</b></div>", unsafe_allow_html=True)
