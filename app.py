...
# --- 4. JÁDRO SIMULACE (S CACHINGEM) ---

def sim_match(t1, t2, match_seed):
    """Pomocná funkce pro simulaci jednoho zápasu se stabilním seedem."""
    if (t1, t2) in real_results: return real_results[(t1, t2)]
    if (t2, t1) in real_results:
        r = real_results[(t2, t1)]
        return r[1], r[0], r[2]

    # Stabilní seed pro konkrétní dvojici v rámci simulace
...
@st.cache_data  # <--- TATO MAGIE ZAJISTÍ STABILITU VÝSLEDKŮ
def run_tourney_cached(seed):
    """Vygeneruje celý turnaj najednou a uloží ho do paměti pro daný seed."""
    matches = []
    ...
    for i, (d, t1, t2) in enumerate(sched):
        # Match seed musí být unikátní pro každý zápas, ale stejný při každém spuštění
        m_seed = seed + i
...
# Zavoláme CACHED funkci - výsledky budou stabilní
all_m = run_tourney_cached(seed)
date_idx = dates_list.index(sel_date)

# Horní panel - Dnešní zápasy
...
if date_idx <= 4:
    ...
    for gn, teams in groups_def.items():
        # Přepočet tabulky k vybranému dni (na základě stabilních dat z all_m)
